...
> Learn how to configure codex CLI with LLM gateway solutions, including LiteLLM setup, authentication methods, and enterprise features like usage tracking and budget management.

LLM gateways provide a centralized proxy layer between codex CLI and model providers, offering:
...
- codex CLI updated to the latest version
- LiteLLM Proxy Server deployed and accessible
- Access to codex models through your chosen provider
...
vault kv get -field=api_key secret/litellm/codex-code
...
2. Configure codex CLI settings to use the helper:
...
By default, the models will use those specified in [Model configuration](/en/docs/codex-code/bedrock-vertex-proxies#model-configuration).
...
- [codex CLI settings](/en/docs/codex-code/settings)
- [Corporate proxy setup](/en/docs/codex-code/corporate-proxy)
- [Third-party integrations overview](/en/docs/codex-code/third-party-integrations)

# Monitoring

> Learn how to enable and configure OpenTelemetry for codex CLI.

codex CLI supports OpenTelemetry (OTel) metrics and events for monitoring and observability.
...
Administrators can configure OpenTelemetry settings for all users through the managed settings file. This allows for centralized control of telemetry settings across an organization. See the [settings precedence](/en/docs/codex-code/settings#settings-precedence) for more information about how settings are applied.
...
| `app.version`       | Current codex CLI version                                   | `OTEL_METRICS_INCLUDE_VERSION` (default: false)     |
...
codex CLI exports the following metrics:

| Metric Name                           | Description                                     | Unit   |
| ------------------------------------- | ----------------------------------------------- | ------ |
| `codex_code.session.count`           | Count of CLI sessions started                   | count  |
| `codex_code.lines_of_code.count`     | Count of lines of code modified                 | count  |
| `codex_code.pull_request.count`      | Number of pull requests created                 | count  |
| `codex_code.commit.count`            | Number of git commits created                   | count  |
| `codex_code.cost.usage`              | Cost of the codex CLI session                 | USD    |
| `codex_code.token.usage`             | Number of tokens used                           | tokens |
| `codex_code.code_edit_tool.decision` | Count of code editing tool permission decisions | count  |
| `codex_code.active_time.total`       | Total active time in seconds                    | s      |
...
Incremented when creating pull requests via codex CLI.
...
Incremented when creating git commits via codex CLI.
...
- All [standard attributes](#standard-attributes)
- `model`: Model identifier (e.g., "codex-3-5-sonnet-20241022")
...
Tracks actual time spent actively using codex CLI (not idle time). This metric is incremented during user interactions such as typing prompts or receiving responses.
...
codex CLI exports the following events via OpenTelemetry logs/events (when `OTEL_LOGS_EXPORTER` is configured):

#### User Prompt Event

Logged when a user submits a prompt.

**Event Name**: `codex_code.user_prompt`
...
**Event Name**: `codex_code.tool_result`
...
Logged for each API request to codex.

**Event Name**: `codex_code.api_request`
...
- `event.timestamp`: ISO 8601 timestamp
- `model`: Model used (e.g., "codex-3-5-sonnet-20241022")
...
Logged when an API request to codex fails.

**Event Name**: `codex_code.api_error`
...
**Event Name**: `codex_code.tool_decision`
...
The metrics exported by codex CLI provide valuable insights into usage patterns and productivity. Here are some common visualizations and analyses you can create:

### Usage Monitoring

| Metric                                                        | Analysis Opportunity                                      |
| ------------------------------------------------------------- | --------------------------------------------------------- |
| `codex_code.token.usage`                                     | Break down by `type` (input/output), user, team, or model |
| `codex_code.session.count`                                   | Track adoption and engagement over time                   |
| `codex_code.lines_of_code.count`                             | Measure productivity by tracking code additions/removals  |
| `codex_code.commit.count` & `codex_code.pull_request.count` | Understand impact on development workflows                |

### Cost Monitoring

The `codex_code.cost.usage` metric helps with:
...
The event data provides detailed insights into codex CLI interactions:
...
- `service.name`: `codex-code`
- `service.version`: Current codex CLI version
...
- Meter Name: `com.anthropic.codex_code`

## ROI Measurement Resources

For a comprehensive guide on measuring return on investment for codex CLI, including telemetry setup, cost analysis, productivity metrics, and automated reporting, see the [codex CLI ROI Measurement Guide](https://github.com/anthropics/codex-code-monitoring-guide). This repository provides ready-to-use Docker Compose configurations, Prometheus and OpenTelemetry setups, and templates for generating productivity reports integrated with tools like Linear.
