...
Typical usage:
    uv run RUNNERS/codex_runner.py --prp test --interactive
    uv run RUNNERS/codex_runner.py --prp test --output-format json
    uv run RUNNERS/codex_runner.py --prp test --output-format stream-json

Arguments:
    --prp-path       Path to a PRP markdown file (overrides --prp)
    --prp            Feature key; resolves to PRPs/{feature}.md
    --model          CLI executable for the LLM (default: "codex") Only codex CLI is supported for now
...
def main() -> None:
    ...
    parser.add_argument("--model", default="codex", help="Model CLI executable name")
    parser.add_argument(
        "--output-format",
